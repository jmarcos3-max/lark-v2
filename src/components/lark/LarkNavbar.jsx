...
import { LogOut, User, Sparkles, RefreshCw, Sun, Moon, Monitor, Loader2, X } from 'lucide-react';
...
function GuestLoginButton() {
  ...
  const handleConnect = () => {
    setShowModal(false);
    setConnecting(true);
    // Simulate Audiotool SDK at.login() OAuth redirect loop
    setTimeout(() => setConnecting(false), 2800);
  };
  ...
  if (connecting) {
    return (
      <div
        ...
        style={{ color: 'var(--lark-violet-bright)', border: '1px solid rgba(139,92,246,0.3)', background: 'rgba(139,92,246,0.08)' }}
      >
        <Loader2 size={12} className="animate-spin" />
        <span>Connecting via Nexus...</span>
      </div>
    );
  }
    ...
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)' }}>
          <div
            className="relative rounded-2xl p-8 flex flex-col items-center gap-5 w-80"
            style={{
              background: 'var(--lark-card)',
              border: '1px solid var(--lark-border)',
          ...
          >
            <button
              ...
              style={{ color: 'var(--lark-text-subtle)', background: 'rgba(128,128,128,0.08)' }}
            >
              <X size={13} />
            </button>
            ...
            <div className="text-center">
              ...
              <p className="text-xs leading-relaxed" style={{ color: 'var(--lark-text-muted)' }}>
                Connect your Audiotool account via the Nexus SDK to sync your projects.
              </p>
            </div>
            ...
            <button
              ...
              onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 30px rgba(139,92,246,0.55)'}
        ...
        {user ? (
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-lg" style={{ background: 'rgba(128,128,128,0.06)', border: '1px solid var(--lark-border)' }}>
              <div
                className={`w-1.5 h-1.5 rounded-full ${isConnected ? 'bg-green-400' : 'bg-gray-500'}`}
                style={isConnected ? { boxShadow: '0 0 6px #4ade80' } : {}}
              />
              <span className="text-[10px] font-medium" style={{ color: isConnected ? '#4ade80' : 'var(--lark-text-muted)' }}>
